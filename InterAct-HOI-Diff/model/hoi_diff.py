...
class HOIDiff(MDM):
    def __init__(self,modeltype, njoints, nfeats, num_actions, translation, pose_rep, glob, glob_rot,
        ...
        self.args = args
        self.objEmbedding = nn.Linear(1024*3, self.latent_dim)
    ...
    def encode_obj(self, obj_bps):
        ...
        obj_bps = obj_bps.view(-1, 1024*3)
        obj_emb = self.objEmbedding(obj_bps) # [bs, d]
        # [1, bs, d]
    ...
    def forward(self, x, timesteps, y=None):
        ...
        x_human, x_obj = x[:,:553], x[:,553:]
        # x_human, x_obj = x[:,:263], x[:,263:]

        # Build embedding vector
        emb = self.embed_timestep(timesteps)  # [1, bs, d]

        obj_emb = self.encode_obj(y['obj_bps']) # [1, bs, d]
        obj_emb += emb
        ...
        xseq_human = torch.cat((emb, obj_emb, x_human), axis=0)  # [seqlen+2, bs, d]
        xseq_human = self.sequence_pos_encoder(xseq_human)  # [seqlen+2, bs, d]
        human_mid = self.seqTransEncoder_start(xseq_human)

        xseq_obj = torch.cat((emb, obj_emb, x_obj), axis=0)
        ...
        if self.args.multi_backbone_split < self.num_layers:
            ...
            dec_output_human, dec_output_obj = self.mutual_attn(human_mid[2:], obj_mid[2:])
            output_human = self.seqTransEncoder_end(torch.cat([human_mid[:2], dec_output_human], 0))[2:]
