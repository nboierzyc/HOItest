...
class GaussianDiffusion:
    ...
    def condition_mean(self, cond_fn, x, t, model_kwargs=None):
        ...
        if is_perturb:
            for _ in range(n_guide_steps):
                ...
                # NOTE hard code
                x[:,:553] = x[:,:553] - tao1 * grad[:,:553] 
                x[:,553:] = x[:,553:] - tao2 * grad[:,553:]    
    ...
    def contact_loss(self, motion, contact_lable, mask):
        ...
        h_motion = motion[:, 0:553].permute(0, 2, 3, 1)
        o_motion = motion[:, 553:].permute(0, 2, 3, 1)
...
class LocalMotionDiffusion(GaussianDiffusion):
    ...
    def training_losses(self, model, mask, t_bar, *args, **kwargs):
        ...
        # hardcode
        loss_mse = self.masked_l2(items['target'][:, :559], items['pred'][:, :559], mask) # mean_flat(rot_mse)
        losses = {}
