...
def get_representation(positions, positions_obj, feet_thre):
    ...
    positions = positions.reshape(len(positions), -1)
    data = np.concatenate([positions[:-1], velocity, feet_l, feet_r, positions_obj[:-1]], axis=-1)
    return data
...
if __name__ == "__main__":
    ...
    joints_num = 77
    # ds_num = 8
    data_dir = './dataset/behave/markers/'
    data_obj_dir = './dataset/behave/sequences_seg/'
    #save_dir1 = './dataset/behave_t2m/new_markers_local/'      # './behave/new_joints/'
    save_dir2 = './dataset/behave_t2m/new_marker_vecs_local/'  # './behave/new_joint_vecs/'
    ...
    for source_file in tqdm(source_list):
        try:
            source_data = np.load(os.path.join(data_dir, source_file,'marker.npy'))
            source_data_obj = np.load(os.path.join(data_obj_dir, source_file, 'object.npy'))

            obj_angles = source_data_obj['angles']
            obj_trans = source_data_obj['trans']
            obj_name = source_data_obj['name']
            ...
            data = get_representation(source_data, obj_data, 0.02)
